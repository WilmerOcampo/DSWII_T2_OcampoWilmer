...
public class PaymentEndpoint {
    ...
    public PaymentList paymentList(@RequestPayload PaymentListRequest request) {
        ...
        Date date = Utils.toDate(request.getPaymentDate());
        List<Payment> payments = this.paymentService.findAllByPaymentDateIs(date);
        ...
        if (!payments.isEmpty()) {
            payments.forEach(obj -> {
                ...
                objTemp.setPaymentDate(Utils.toXmlGregorianCalendar(obj.getPaymentDate()));
